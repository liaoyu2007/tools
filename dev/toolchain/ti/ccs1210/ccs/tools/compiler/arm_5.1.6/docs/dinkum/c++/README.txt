...
The Dinkum C/C++ Library Reference is Copyright (c) 1989-2003
by P.J. Plauger. This code is protected by copyright. ALL
RIGHTS RESERVED.

Version 4.02. This set of files constitutes the entire Dinkum
C/C++ Library Reference. It is for use only by the licensed
user. See the files crit_pb.html or crit_pjp.html (reachable via
the Copyright hot link at the bottom of every page) for detailed
restrictions.
...
Extract this archive into any directory. The home page for the
entire Reference is the file index.html. Under Windows, you
can create a desktop icon with Windows Explorer. Right click on
the display for index.html and choose Create Shortcut. Right
click on the display for the created shortcut and choose Rename.
You can then enter a more meaningful name for the icon, such as
``Dinkum C/C++ Reference.'' Drag the shortcut onto the desktop and
drop it there.

If you find that none of the hot links work from the home page,
check the file names. A system that supports long file names
generally requires that the names of web pages end in .html.
Some older utilities truncate these endings to .htm, however.
The DOS/Windows command:

ren *.htm *.html

makes all the necessary changes.
...
This Dinkum® Library References in machine-readable or printed
form (“Dinkum Reference”), hereafter in whole or in part the
“Product”, is copyright © 1989-2002 by P.J. Plauger. ALL RIGHTS
RESERVED. The Product is derived in part from books copyright
© 1992-2001 by P.J. Plauger.
